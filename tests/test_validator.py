...
"""Tests for extension validator instance-level rules.

v2.0.0: Legacy V4/V5/V6/V7/V10/V11 tests removed with ChatExtension. Their
intent (no direct LLM imports, typed params + returns) is now enforced
structurally: Webbee Narrator is the sole voice surface so extensions have
no runtime path to call an LLM provider; the kernel loader rejects
v1-era classes via V14 (``imperal_sdk.validators.v14_no_chatext``).
"""
...
# V4/V5/V6/V7/V10/V11 legacy rules were removed with ChatExtension in SDK
# v2.0.0. Their intent is now enforced structurally:
#   - V4 (action_type), V10 (event for writes), V11 (docstring): previously
#     validated @chat.function metadata; ChatExtension + @chat.function are
#     gone in v2.
#   - V5 (must return ActionResult), V6 (params must be Pydantic): superseded
#     by @ext.tool(output_schema=...) which requires a Pydantic class at
#     decoration time (extension.py, I-TOOL-OUTPUT-SCHEMA-REQUIRED).
#   - V7 (no direct anthropic/openai import): Webbee Narrator is the sole
#     kernel-side LLM caller in v2; extensions have no runtime path to
#     reach a provider even if they import one.
#
# V14 (source-tree AST scan in imperal_sdk.validators.v14_no_chatext)
# rejects v1-era patterns at deploy time — see tests/test_validator_v14.py.
