...
5-step algorithm: DISCOVER → GATE → DISPATCH → TRIAGE → RETRY → COMBINE

Dispatch rules:
- Capabilities query → navigation mode (show ALL capabilities)
- Embedding GATE: no tools match (max_score < 0.25) → navigate (deterministic, no LLM)
...
- LLM routing refines which extension(s) to dispatch
...
from imperal_sdk.runtime.template_resolver import resolve_template
...
# KERNEL GATE threshold: if ALL embedding scores are below this, skip LLM routing
# and go straight to navigate. Embeddings are language-independent (~50ms).
# This prevents greetings/small talk from wasting an LLM routing call.
_NAVIGATE_THRESHOLD = 0.25
...
- For user/role/extension management, system health, automations, platform config → admin
- For multiple topics (emails AND cases) → list both
- For greetings, small talk, general questions about the system, or questions that no extension handles → none
- SHORT MESSAGES like "yes", "ok", "do it", "go ahead" are FOLLOW-UPS to the previous conversation. Route to the SAME extension as the previous request.
...
async def handle_hub_chat(tool_input: dict, execute_fn, catalog, relations: dict) -> dict:
    ...
    if _is_capabilities_query(message):
        return await _hub_navigate(message, history, user_info, catalog, _user_allowed_apps, skeleton=skeleton, context=context)
    ...
    # ── KERNEL GATE: embeddings decide if message is actionable ───
    # discover_tools uses semantic embeddings — fast (~50ms), language-independent.
    # If no tool scores above threshold → navigate directly (no LLM routing cost).
    # This prevents greetings, small talk, "Привет!", "Thanks" from reaching extensions.
    # EXCEPTION: automation messages bypass gate (they use _detect_automation_target).
    # EXCEPTION: follow-up messages (<=6 words with session state) bypass gate —
    #            they need LLM routing to resolve "do it" → last extension.
    is_automation = bool(context.get("automation_rule_id"))
    _max_score = max((t.get("relevance", 0) for t in found_tools), default=0)
    _is_short_followup = len(message.strip().split()) <= _FOLLOWUP_MAX_WORDS

    if not is_automation and not _is_short_followup:
        if not found_tools or _max_score < _NAVIGATE_THRESHOLD:
            log.info(f"Hub KERNEL GATE: no tools matched (max_score={_max_score:.3f} < {_NAVIGATE_THRESHOLD}) → navigate")
            return await _hub_navigate(message, history, user_info, catalog, _user_allowed_apps, skeleton=skeleton, context=context)

    # ── Step 1b: LLM ROUTING — refine extension selection + intent ─
    # Embeddings found candidates above threshold. LLM routing picks the
    # best extension(s) and classifies intent (read/write/destructive).
    ...
    if not is_automation and catalog and catalog.loaded:
        ...
        if llm_app_ids:
            ...
            # LLM routing returned no extension (none) — navigate
            # This handles edge cases where embeddings scored above threshold
            # but the message is still conversational (LLM understands nuance).
            log.info(f"Hub LLM routing returned none → navigate (embeddings had max_score={_max_score:.3f})")
            return await _hub_navigate(message, history, user_info, catalog, _user_allowed_apps, skeleton=skeleton, context=context)
    ...
    if not extensions:
        ...
        if _is_auto_msg and _has_auto_scope:
            ...
            if isinstance(result, dict):
                result["_routed_intent"] = ""
                return result
            return {"response": str(result), "_handled": True, "_routed_intent": ""}
        return await _hub_navigate(message, history, user_info, catalog, _user_allowed_apps, skeleton=skeleton, context=context)
    ...
    if len(extensions) == 1:
            ...
            for fallback_app, fallback_candidate in all_extensions.items():
                if fallback_app != app_id:
                    ...
                    if fb_handled and not _is_extension_error(fb_text):
                        ...
                        _fb_result["_routed_intent"] = context.get("_intent_type", "")
                        return _fb_result
            # All fallbacks failed — navigate
            # Navigate fallback after failed dispatch — preserve original routing intent
            log.info(f"Hub navigate fallback: preserving _routed_intent={context.get('_intent_type', '')}")
            _nav_result = await _hub_navigate(message, history, user_info, catalog, _user_allowed_apps, skeleton=skeleton, context=context)
            _nav_result["_routed_intent"] = context.get("_intent_type", "")
            return _nav_result
        context.pop("_suppress_promotion", None)
        ...
        _result["_routed_intent"] = context.get("_intent_type", "")
        return _result
    ...
    chain_context = {}  # Structured variable passing between chain steps
    _truth_flags = {}
    ...
    for _chain_idx, (app_id, step_message) in enumerate(chain_steps):
        ...
        # Resolve {{prev.data.X}} / {{steps.N.data.X}} templates in step message
        if chain_context:
            _resolved_step_msg = resolve_template(step_message, chain_context)
        else:
            _resolved_step_msg = step_message

        enriched_msg = f"{_chain_ctx}\n\n{_resolved_step_msg}" if _chain_ctx else _resolved_step_msg
        
        # Suppress task promotion in chain mode — we need all results for combine.
        # Detect intent per chain step from step message keywords.
        _step_lower = step_message.lower()
        _write_words = ("send", "create", "reply", "forward", "compose", "save", "update",
                        "отправь", "создай", "ответь", "перешли", "сохрани", "обнови")
        _destructive_words = ("delete", "remove", "archive", "trash",
                              "удали", "убери", "заархивируй")
        if any(w in _step_lower for w in _destructive_words):
            context["_intent_type"] = "destructive"
        elif any(w in _step_lower for w in _write_words):
            context["_intent_type"] = "write"
        else:
            context["_intent_type"] = "read"
        context["_suppress_promotion"] = True
        ...
        # ── Chain Variable Passing: extract ActionResults from step ──
        from imperal_sdk.chat.action_result import ActionResult  # lazy import to avoid circular
        _step_funcs = result.get("_functions_called", []) if isinstance(result, dict) else []
        _step_action_results = [f.get("result") for f in _step_funcs
                                if f.get("result") and isinstance(f.get("result"), (ActionResult, dict))]

        if _step_action_results:
            # Merge data from ALL functions in this step
            _merged_data = {}
            for _ar in _step_action_results:
                if isinstance(_ar, ActionResult):
                    _merged_data.update(_ar.data)
                elif isinstance(_ar, dict):
                    _merged_data.update(_ar.get("data", {}))

            _last_ar = _step_action_results[-1]
            _step_ctx = {
                "status": _last_ar.status if isinstance(_last_ar, ActionResult) else _last_ar.get("status"),
                "data": _merged_data,
                "summary": _last_ar.summary if isinstance(_last_ar, ActionResult) else _last_ar.get("summary", ""),
            }
            chain_context[f"steps.{_chain_idx + 1}"] = _step_ctx
            chain_context["prev"] = _step_ctx

            # Stop chain on error — no point continuing if step failed
            _last_status = _last_ar.status if isinstance(_last_ar, ActionResult) else _last_ar.get("status")
            if _last_status == "error":
                log.warning(f"Chain stopped at step {_chain_idx + 1}: error in {app_id}")
                break

        log.info(f"Hub chain: {app_id} step {_chain_idx+1} done, passing context to next")
    
    if not successful:
        # Navigate fallback after failed chain — preserve original routing intent
        log.info(f"Hub navigate fallback (chain): preserving _routed_intent={context.get('_intent_type', '')}")
        _nav_result = await _hub_navigate(message, history, user_info, catalog, _user_allowed_apps, skeleton=skeleton, context=context)
        _nav_result["_routed_intent"] = context.get("_intent_type", "")
        return _nav_result
    ...
    combined["_routed_intent"] = context.get("_intent_type", "")
    return combined
...
async def _hub_navigate(message: str, history: list, user_info: dict, catalog, allowed_apps: set = None, skeleton: dict = None, context: dict = None) -> dict:
    ...
    # ── Fix 2: Time context for navigate ──────────────────────────
    _time_info = ""
    _ctx = context or {}
    _time_ctx = _ctx.get("_time", {})
    if _time_ctx:
        _time_info = f"\nCurrent time: {_time_ctx.get('now_local', 'unknown')} ({_time_ctx.get('timezone', 'UTC')})"

    # ── Fix 3: Skeleton summary for proactive navigate ────────────
    _user_data_summary = ""
    _skel = skeleton or {}
    _summary_parts = []
    # Email accounts
    _email_accts = _skel.get("email_accounts", [])
    if _email_accts:
        _acct_lines = []
        for _a in _email_accts:
            _e = _a.get("email", "")
            _unread = _a.get("unread", _a.get("unread_count", ""))
            if _e:
                _acct_lines.append(f"  - {_e}" + (f" ({_unread} unread)" if _unread else ""))
        if _acct_lines:
            _summary_parts.append("Email accounts:\n" + "\n".join(_acct_lines))
    # Notes
    if _skel.get("notes"):
        _notes = _skel["notes"]
        if isinstance(_notes, dict):
            _nc = _notes.get("count", _notes.get("total", ""))
            if _nc:
                _summary_parts.append(f"Notes: {_nc} notes")
        elif isinstance(_notes, list):
            _summary_parts.append(f"Notes: {len(_notes)} notes")
    # Cases (sharelock)
    if _skel.get("sharelock-v2") or _skel.get("cases"):
        _cases = _skel.get("sharelock-v2") or _skel.get("cases")
        if isinstance(_cases, dict):
            _cc = _cases.get("count", _cases.get("total", ""))
            if _cc:
                _summary_parts.append(f"Cases: {_cc} cases")
    if _summary_parts:
        _user_data_summary = "\n\n[USER CONTEXT \u2014 use for proactive greetings]\n" + "\n".join(_summary_parts)

    system_prompt = f"""You are Imperal Cloud — the world's first AI Cloud Operating System powered by ICNLI.

You are NOT a helper, NOT a navigator, NOT a chatbot. You ARE the OS. You can do EVERYTHING your extensions can do — manage users, read emails, create notes, investigate cases, send messages, set up automations. You do it all. You are like Jarvis — one intelligent system that handles everything.

CURRENT USER: {user_email} (role: {user_role}){_time_info}
...
IDENTITY RULES:
1. You are Imperal Cloud. NEVER say "I'm the Notes helper" or "I'm the Gmail assistant" or "ask extension X". YOU are Imperal Cloud and you handle EVERYTHING.
2. When asked "what can you do?" — list ALL capabilities naturally. These are YOUR capabilities, not separate extensions.
3. When asked to do something — say "I'll do it" or "Done" or describe the result. NEVER redirect to another extension.
4. If a request wasn't handled properly, apologize briefly and ask to rephrase. NEVER mention internal extensions by name.
5. NEVER fabricate capabilities. Only list what's in the catalog above.
6. Respond in the user's language. No emojis. "Imperal" not "Imperial".
7. Be confident, concise, and intelligent. You are an AI Cloud OS, act like one.
8. CRITICAL: You are in NAVIGATION MODE — you can ONLY have a conversation. You CANNOT perform actions (send emails, delete files, create notes, etc.) in this mode. NEVER claim to have performed an action. If user asks to DO something, say "Let me handle that" and the system will route to the right capability automatically.
9. NEVER say "sent", "deleted", "archived", "created", "done" about any action. You have NO action capability in this mode.
10. For GREETINGS ("Hi", "Hello", "Привет"): if USER CONTEXT is available, be PROACTIVE — mention their accounts, unread counts, recent activity. Be a smart OS that knows its user. Example: "Привет! У тебя 2 email аккаунта, 5 непрочитанных писем. Чем могу помочь?"""
    ...
    try:
        ...
        return {"response": text, "_routed_intent": ""}
    except Exception as e:
        log.error(f"Hub navigate error: {e}")
        return {"response": "Hello! I'm the Imperal Cloud assistant. How can I help you?", "_routed_intent": ""}
